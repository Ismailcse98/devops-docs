...
Delete All Running + Stopped Container
```bash
...
Delete All Container + Image + Network + Build Cache
```bash
docker system prune -a -f
```

Delete All Container + Image + Network + Build Cache With Volumes
```bash
docker system prune -a --volumes -f
```
Stop + Remove Container + Remove Image in One Line
```bash
docker stop $(docker ps -q) && docker rm -f $(docker ps -aq) && docker rmi -f $(docker images -aq)
```

Check your Container + Image + Network + Volumes
```bash
docker ps -a
docker images
docker volume ls
